...
def send(template, subject, recipients, **templatevars):
    ...
    plaintext_template = '%s%stemplates/emails/%s.txt' % (path, os.path.sep, template)
    html_template = '%s%stemplates/emails/%s.html' % (path, os.path.sep, template)
    msg = Message(subject,
    ...
    if os.path.exists(plaintext_template):
        msg.body = render_template('emails/%s.txt' \
                % template, **templatevars).encode('ascii', 'ignore')
    else:
    ...
    if os.path.exists(html_template):
        msg.html = render_template('emails/%s.html' \
                % template, **templatevars).encode('ascii', 'ignore')
    else:
